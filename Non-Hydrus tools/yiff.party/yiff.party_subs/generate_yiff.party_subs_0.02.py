...
#Rename old favorite list to the blacklist name before adding another.
#This way it will make subs already exported.
blacklist_file = './yiff_blacklist.txt'
...
if os.path.isfile(blacklist_file):
    bline = open(blacklist_file,'r').readline().strip()
    if bline.startswith('['): bline = bline[1:]
    if bline.endswith(']'): bline = bline[:-1]
    blacklist = [i.strip() for i in bline.split(',')]
